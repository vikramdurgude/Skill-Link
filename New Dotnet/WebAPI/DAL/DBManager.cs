...
public class DBManager{
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    conn.Open();
                     ...
                     string query = @"SELECT DISTINCT ur.RequirementID, u.UserID, u.NameFirst, u.NameLast, u.PhoneNumber, ur.Skills, ur.Wages, ur.Address, ur.Date
                                    FROM Users u
        ...
        public static bool AddBookingEntry(int RequirementID,int userID, BOL.ServiceProvider serviceProvider)
        {
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    string query = @"INSERT INTO BookingList (RequirementID,UserID,Username,NameFirst, NameLast, PhoneNumber, Skills, Wages) 
                                    VALUES (@RequirementID,@UserID,@Username, @NameFirst, @NameLast, @PhoneNumber, @Skills, @Wages)";
    ...
    public static List<BookingList> GetBookingListByUserId(int userId){
        ...
        string query = @" SELECT b1.RequirementID,b1.UserID,b1.Username,b1.NameFirst, b1.NameLast,b1.PhoneNumber,b1.Skills,
    ...
    public static bool SelectServiceProvider(int userId, string serviceProviderUsername, bool isSelected){
        using (MySqlConnection connection = new MySqlConnection(connString))
        {
            try
            {
                ...
                string query = "INSERT INTO Status (RequirementID, ServiceProviderUserName, IsSelected) VALUES (@UserID, @ServiceProviderUserName, @IsSelected)";
    ...
    public static List<ServiceProviderViewStatus> GetServiceProviderData(string serviceProviderUsername){
            ...
            using (MySqlConnection connection = new MySqlConnection(connString))
            {
                string query = @"SELECT distinct u.NameFirst, u.NameLast, u.PhoneNumber, ur.Wages, ur.Address, ur.Date
                                 FROM users u
                                 INNER JOIN userrequirements ur ON u.UserID = ur.UserID
                                 INNER JOIN status st ON u.UserID = st.RequirementID
                                 INNER JOIN serviceproviders ON ur.Skills = serviceproviders.Skills
